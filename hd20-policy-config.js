...
const DEFAULT={auditRisk:{enabled:false,weights:null},improvementDeadline:{minDays:7,maxDays:14,defaultDays:null}};
...
function riskWeights(){const p=read().auditRisk,w=p.weights;if(!p.enabled||!w||typeof w!=='object')return null;const keys=['previousMonth','cumulative','overdue','recurrence'];const out={};for(const k of keys){const n=Number(w[k]);if(!Number.isFinite(n)||n<0)return null;out[k]=n}return out}
window.HD20PolicyConfig={KEY,DEFAULT,read,write,deadlineDays,riskWeights};
})();