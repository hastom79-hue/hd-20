...
function bind(){document.addEventListener('click',e=>{const manual=e.target?.closest?.('#hd20ActionRegisterBtn');if(manual){clear();return}const m=modal();if(!m||!m.classList.contains('on'))return;if(e.target===m||e.target?.closest?.('.armClose'))setTimeout(clear,0)},true);document.addEventListener('keydown',e=>{if(e.key==='Escape'&&modal()?.classList.contains('on'))setTimeout(clear,0)},true)}
