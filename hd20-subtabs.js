...
function applyAdvancement(sub){const conversion=$('#performanceConversionAnalysis'),work=$('#awWorkplace');if(sub==='judge'){show(conversion,true);show(work,true)}else{show(conversion,false);show(work,true)}if(work){work.dataset.subview=sub;showAll(work);show($('.awFlow',work),true);show($('.awGrid',work),true)}if(conversion)conversion.dataset.subview=sub}
