...
function applyActivity(sub){const root=$('#awActivity');if(!root)return;root.dataset.subview=sub;showAll(root);show($('.awFlow',root),true);show($('.awGrid',root),true);const cards=$$('.awCard',root);if(sub==='analysis'&&cards.length>1)show(cards[0],false)}
function applyAdvancement(sub){const conversion=$('#performanceConversionAnalysis'),work=$('#awWorkplace');document.querySelector('.app')?.classList.add('awFocused');if(work){work.classList.add('on');show(work,true);work.dataset.subview=sub;showAll(work);show($('.awFlow',work),true);show($('.awGrid',work),true)}if(conversion){conversion.classList.toggle('on',sub==='judge');show(conversion,sub==='judge');conversion.dataset.subview=sub}}
function applyAudit(sub){const root=$('#awAudit');if(!root)return;root.dataset.subview=sub;showAll(root);show($('.awFlow',root),true);show($('.awAuditLane',root),true);const cards=$$('.awCard',root);if(sub==='retention'&&cards.length)show(cards[0],false)}
function applyAction(sub){const root=$('#awAction');if(!root)return;root.dataset.subview=sub;showAll(root);show($('.awFlow',root),true);show($('.awKpis',root),true);const cards=$$('.awCard',root);if(sub==='verify'&&cards.length)show(cards[0],false)}
