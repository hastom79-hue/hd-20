...
function apply(detail){
  ...
  const sub=detail.sub==='draw'||detail.sub==='inspect'||detail.sub==='retention'?detail.sub:'draw';
  root.dataset.canonicalSubview=sub==='retention'?'retention-six-month':sub==='inspect'?'audit-inspect':'audit-draw';
  const lane=root.querySelector(':scope > .awAuditLane');
  if(lane)lane.classList.toggle('hd20SubHidden',sub!=='retention');
