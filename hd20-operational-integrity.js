...
function run(){if(running)return;running=true;try{patchMetrics();patchTrace()}finally{running=false}}
function schedule(){if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;run()})}
function bind(){['hd20-subtab-changed','hd20-action-updated','hd20-refresh-requested','storage'].forEach(ev=>window.addEventListener(ev,schedule));const mo=new MutationObserver(schedule);mo.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['class']});setTimeout(schedule,300)}
window.HD20_OPERATIONAL_INTEGRITY={seoulDate,overdue,effectVerified,recurrence,completedVerified,completedRecurrence,patchNativeActionSummary,run,schedule};document.readyState==='loading'?document.addEventListener('DOMContentLoaded',bind,{once:true}):bind();
})();