...
function run(){if(running)return;running=true;try{patchActivityDate();patchMetrics();patchTrace()}finally{running=false}}
...
window.HD20_OPERATIONAL_INTEGRITY={seoulDate,utcDate,overdue,effectVerified,recurrence,completedVerified,completedRecurrence,patchActivityDate,patchNativeActionSummary,run,schedule};document.readyState==='loading'?document.addEventListener('DOMContentLoaded',bind,{once:true}):bind();
})();