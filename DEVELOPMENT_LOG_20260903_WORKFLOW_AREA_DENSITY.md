...
## Header / 5영역 Navigation 공통 스케일 정규화
- `hd20-overhaul.css`의 상단 Header를 축소해 업무영역 진입 전 차지하는 세로공간을 줄였다.
- 시스템명은 Desktop에서 한 줄을 유지하고 좁은 화면에서는 정상 wrap되도록 했다.
- Utility 버튼 높이/간격을 통일하고 Header control 군의 과도한 폭을 줄였다.
- 5영역 Navigation은 Desktop에서 5열을 유지하되 높이·padding·폰트 크기를 compact하게 조정했다.
- 1280px 이하에서 글자/설명 밀도를 한 단계 낮추고, 980px 이하 3열, 700px 이하 2열, 460px 이하 1열로 단계적으로 전환한다.
- 기존 820px 지점에서 갑자기 5열→2열로 꺾이던 반응형을 완화해 125% 확대 및 중간폭 화면에서 Navigation이 과도하게 커지는 현상을 줄였다.
- Dashboard/②~⑤ 업무 데이터, 산식, 탭 동작, 업무기준은 변경하지 않았다.
...
- `0187634` — Header / 5영역 Navigation 공통 스케일·반응형 정규화
