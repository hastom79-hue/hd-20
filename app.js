const rawData = [
 ...
 {no:4,type:"시각화관리",item:"표준과 이상상태를 누구나 즉시 구분할 수 있는가?",issue:"공구 위치 표시 불명확",action:"쉐도우보드 및 위치 라벨 적용",status:"완료"},
 {no:5,type:"자주보전",item:"설비 주변 누유·누수·이상소음 점검이 정례화 되어 있는가?",issue:"확인주기 불명확",action:"일일점검 기준 신규 설정",status:"미결"}
];
const standards = [
 ...
 ["5","시각화관리","1","01. 표준/정상/이상 상태가 시각적으로 구분되는가?","","사용","SYSTEM","2026-01-19"],
 ["6","자주보전","1","01. 설비의 누유/누수/이상소음 확인이 이루어지는가?","","사용","SYSTEM","2026-01-19"]
];
const audits = [
 ...
 ["4","시각화관리","정리·정돈·청소를 유지 개선","표준과 이상을 즉시 구분할 수 있는가?","5 / 5","공구 위치 표시 불량","쉐도우보드 적용","완료"]
];
...
function renderStacks(){
 const teams=[["조립1팀",[5,4,3,2,1]],["Rear조립팀",[4,2,2,3,1]],["가공1팀",[2,3,4,1,2]],["자재운영팀",[1,2,1,2,1]]];
 const el=document.getElementById("issueChart");el.innerHTML="";
 teams.forEach(([name,arr])=>{const sum=arr.reduce((a,b)=>a+b,0);el.innerHTML+=`<div class="stack-row"><b>${name}</b><div class="stack-track">${arr.map((x,i)=>`<span class="seg s${i+1}" style="width:${x/sum*100}%"></span>`).join("")}</div><strong>${sum}</strong></div>`})
}
