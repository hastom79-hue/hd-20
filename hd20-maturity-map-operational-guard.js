...
function sortCases(){const root=$(ROOT);if(!root)return false;$$('.mmtTeamCases',root).forEach(box=>{const rows=$$('.mmtCase',box),ordered=[...rows].sort((a,b)=>dateKey(b).localeCompare(dateKey(a)));if(ordered.some((node,i)=>node!==rows[i]))ordered.forEach(x=>box.appendChild(x))});const hint=$('.mmtCaseCard .mmtHead span',root);if(hint&&hint.textContent!=='팀 선택 → 최신 판정순 Case 상세')hint.textContent='팀 선택 → 최신 판정순 Case 상세';return true}
