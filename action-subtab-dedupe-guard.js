...
function classify(el){
  ...
  if(el.matches('.amSummary,.amGrid,.amRecentList,.amCasesScroll,.hd20LtWrap'))return'manage';
...
function apply(detail){
  ...
  const sub=detail.sub==='master'||detail.sub==='verify'?detail.sub:'manage';
  root.dataset.canonicalSubview=sub==='verify'?'effect-recurrence':sub==='master'?'team-leader-master':'action-management';
