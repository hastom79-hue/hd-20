...
const MAP={
 ...
 'Audit 후 6개월 유지율':{note:'Audit 실시 후 6개월간 지속관리된 대상 중 종료평가에서 요구수준을 유지한 비율',heads:['생산팀','Audit 실시일','6개월 관리종료일','개선요청','기한경과·재발','종료평가']},
 'Audit 부적합 재발률':{note:'조치 완료된 부적합 중 동일·유사 문제가 다시 발생한 비율',heads:['생산팀','작업장·사례','최초 부적합','조치완료일','재발여부','재발일','유형']},
 '기한 내 개선조치 완료율':{note:'Audit 개선요청을 등록일 기준 운영정책으로 자동 지정된 완료기한 안에 Close한 비율',heads:['생산팀','개선조치','등록일','자동 완료기한','완료일','기한준수','상태']}
};
...
function patchModal(){
 ...
 for(const m of modals){const txt=norm(m.innerText);const hit=Object.entries(MAP).find(([k])=>txt.includes(k));if(!hit)continue;const [,cfg]=hit;
  let note=m.querySelector('.hd20DecisionNote');if(!note){note=document.createElement('div');note.className='awHint hd20DecisionNote';const anchor=m.querySelector('.modalHead,.kpiModalHead,.modal-header,h2,h3');(anchor?.parentNode||m).insertBefore(note,anchor?.nextSibling||m.firstChild)}note.textContent=cfg.note;
  const th=[...m.querySelectorAll('table thead th')];if(th.length){cfg.heads.forEach((h,i)=>{if(th[i])th[i].textContent=h})}
 }
}
function boot(){document.addEventListener('click',()=>setTimeout(patchModal,40),true);patchModal()}
