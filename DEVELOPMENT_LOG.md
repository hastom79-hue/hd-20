...
- Commit: `8c5129c`(Audit 오분류·isNonProdRow·팀마스터·fixture v10),
  ...
  `b8f75bc`(고도화 맵 DOM 삽입 배치화·종료평가 race 완화).
...
- 잔여사항: `scale=3` 대용량에서 일부 뷰 8~33초 소요, 고도화 맵 버블 클릭 반응
  속도(28개 MutationObserver 전수 감사 필요), 종료평가 탭 간헐적 race(빈도는
  낮췄으나 완전 제거는 아님) — 모두 성능 후속 과제로 이월. 외부 뉴스 이미지
  리소스 2건 실사용 브라우저 미확인.

### 2026-09-23(같은 세션 연속) / 대시보드 지표 최우선 배치
- 요구사항: "대시보드탭에서는 지표가 가장 최우선으로 디스플레이되도록
  레이아웃을 개선해줘". 헤드라인 KPI 5개 카드(.cards)가 OPERATION HEALTH
  상세 블록(1,228px)보다 뒤에 있던 것을 발견해 순서를 반전.
  `dashboard-priority-layout.js`의 삽입 지점을 `beforebegin`→`afterend`로
  변경. 조사 중 잘못된 회귀 의심(Subnav/PurposePanel/OpsMetrics 소실)이
  있었으나 원본 파일 대조로 무관함을 반증.
- Commit: `d4ca925`(대시보드 지표 우선 배치).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일(성능 후속 과제 3건, 외부 이미지 미확인).

### 2026-09-23(같은 세션 연속 2) / 나머지 15개 탭 우선순위 재배치
- 요구사항: "다른 세부탭들도 정보 우선순위에 따라 레이아웃 재배치해줘".
  15개 탭 전수 조사 결과 대시보드와 동일 패턴은 ③ 확정·수평전개 1곳
  ("운영상태" 요약이 154행 상세표 뒤에 위치) → 순서 반전. 조사 중
  ④ 6개월 관리중의 유사 패턴은 레거시 코드 의심으로 보류. 별도로 ⑤
  효과·재발관리 탭의 핵심 콘텐츠가 앵커 클래스명 불일치로 항상 렌더링
  안 되고 있던 실제 버그를 발견·수정(레이아웃 이전에 콘텐츠 자체가
  없었음).
- Commit: `80b1fd2`(확정·수평전개 순서 반전 + 효과·재발관리 렌더링 버그
  수정).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일 + ④ 6개월 관리중 awAuditLane 순서 재조사 필요.

### 2026-09-23(같은 세션 연속 3) / 스크롤바로 인한 좌우 쏠림 보정
- 요구사항: "스크롤바 내리면서 한쪽으로 쏠림과 치우침 여백 생김 보정해줘".
  `.app{margin:auto}`가 스크롤바 유무에 따라 달라지는 뷰포트 폭 기준으로
  가운데 정렬되어, 탭 길이(스크롤 유무)가 바뀔 때마다 좌우 여백이
  비대칭해지던 문제. 헤드리스 브라우저 기본(오버레이 스크롤바)에서는
  재현 안 돼 `--disable-features=OverlayScrollbar`로 클래식 스크롤바를
  강제해 실측·검증. `html{scrollbar-gutter:stable both-edges}` 추가로
  완전 대칭 확보(수정 전 진단 단계에서 both-edges 없이 stable만 썼을 때
  15px 고정 비대칭이 남는 것도 실측으로 확인 후 보정).
- Commit: `abc91fc`(스크롤바 거터 예약으로 좌우 대칭 보정).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일.

### 2026-09-23(같은 세션 연속 4) / 탭별 잔여 중복 재검토
- 요구사항: "아직도 탭별 중복이 보이고 깔끔하게 분석되지 않고 덜 다듬어
  진거 같아... 정리가 필요해보이는게 많고". 16개 탭 풀페이지 스크린샷
  육안 재검토로 ② 활동관리의 WORK PURPOSE 패널과 자체 awHero 문단이
  중복 반복되는 것을 발견·제거. 같은 패턴이 ③④⑤ 소스에도 있었으나
  실제로는 렌더링되지 않는 죽은 코드임을 스크린샷 대조로 확인 후 ②만
  수정.
- Commit: `3f6be0f`(활동관리 소개 문구 중복 제거).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일. 사용자 언급대로 다른 "정리가 필요한" 부분이
  더 있을 수 있어 후속 피드백 대기.

### 2026-09-23(같은 세션 연속 5) / 2·3영역 전면 개편 요청 대응
- 요구사항: "2.활동관리, 3.고도화 표준화 탭 전체적으로 혼재되어 있어
  눈에 들어오지 않는다 전면 개편하라". 6개 서브탭 전체를 풀페이지
  스크린샷으로 재검토해 2건의 구조적 중복을 발견·제거:
  1) 후보 목록: `#awWorkplace`가 WORK PURPOSE 패널·상단 metric 카드와
     겹치는 자체 소개문·조건배너·"운영상태" 요약카드를 154행 표 앞에
     다시 그리고 있던 것 제거.
  2) 활동관리: "활동 실적·개선이력" 카드 내부 `.awKpis`가 상단
     `#hd20OpsMetrics` 스트립과 완전히 같은 4개 숫자를 반복하던 것 제거.
  사용자에게 남은 애매한 부분(퍼널 중복 항목 처리 여부)을 질문.
- 이어서 "전직원이 다 쓸 웹으로 화면 설명이 너무 어려운 구성이다"라는
  후속 요구사항에 따라, 화면 전반의 영어 전문용어 소제목 7종(WORK
  PURPOSE/FIELD EXECUTION/ADVANCEMENT CONTROL/PERFORMANCE FLOW/
  OPERATION HEALTH/ADVANCEMENT PORTFOLIO/Canonical Source)을 쉬운
  한국어로 전면 교체하고, 개념 나열형 설명문 1건을 행동 중심 문장으로
  재작성. 5단계 퍼널의 중복 항목 제거는 `확정·수평전개` 탭과 마크업을
  공유하고 있어 리스크가 확인돼 별도 과제로 보류.
- Commit: `cf11419`(구조적 중복 2건 제거), `7b70f79`(쉬운말 전환 7건).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전 항목 + 5단계 퍼널의 "②고도화후보"/"③공식확정" 중복
  항목을 서브탭별로 정밀하게 숨기는 작업(확정·수평전개 탭과의 markup
  공유 문제 해결 필요).

### 2026-09-23(같은 세션 연속 6) / "삭제 아닌 재구성" 원칙 확인 후 재배치
- 요구사항: "너무 이거저것 혼재되어 있어 그렇다고 아무거나 삭제하라는게
  아니야" → 사용자에게 방식을 질문 → "같은 주제끼리 묶고 카드/섹션으로
  명확히 구분(내용은 유지)" 확정.
- ② 활동관리의 실제 DOM을 직접 쿼리해 재확인한 결과, "중복 카드"로
  보였던 것은 착시였고(`workflow-crud.js`가 기존 `.awActions`에 버튼만
  병합), 진짜 문제는 Excel Import 관련 두 블록이 원래 `.awHero` 바로
  다음에 와야 할 4단계 흐름가이드(`.awFlow`)를 갈라놓고 있던 것이었음.
  `activity-excel-preview-import.js`의 삽입 앵커를 `.awHero`→`.awFlow`
  로 한 줄 변경해 콘텐츠 삭제 없이 "안내→데이터 가져오기→표→분석"
  순서로 재배치.
- Commit: `03ec959`(활동관리 주제별 재배치).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일(퍼널 중복 처리는 이번엔 "삭제"가 아니라 "재구성"
  관점에서 다시 설계 필요) + ③ 고도화·표준화 쪽의 같은 방식 재배치는
  아직 미착수.

### 2026-09-23(같은 세션 연속 7) / 고도화·표준화 재배치
- 요구사항: "네"(③ 고도화·표준화도 같은 방식으로 재배치 진행 확인).
  `#performanceConversionAnalysis`의 실제 DOM 순서를 조회해, 진행현황
  퍼널 직후에 와야 할 실제 후보목록 표(154행, 핵심 데이터)가 팀별현황·
  조건배너보다 뒤(맨 마지막)에 있던 것을 발견. `integrateAdvancement()`
  의 삽입 지점을 `.pcInsight`/`.pcFlow` 뒤로 한 곳만 변경해 "안내→진행
  현황→실제목록→팀별/조건분석" 순서 완성(콘텐츠 삭제 없음). 이 컴포넌트를
  공유하는 3개 서브탭 전부 표시/숨김 로직 영향 없음을 스크린샷으로 확인.
- Commit: `7de480a`(고도화·표준화 주제별 재배치).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일 + 퍼널 중복 항목의 "재구성"(삭제 아닌) 관점
  재설계는 여전히 미착수.

### 2026-09-23(같은 세션 연속 8) / 퍼널 중복을 "연결 설명"으로 재해결
- 요구사항: "네"(퍼널 중복 항목 처리를 "삭제 아닌 재구성" 원칙으로
  이어서 진행 확인). 마크업 삭제는 확정·수평전개 탭 필터링과 공유되어
  위험함을 재확인 → 대신 `.pcHeader` 설명문에 "이 퍼널이 위 핵심지표
  카드와 같은 데이터를 흐름으로 보여준다"는 연결 문장 한 줄 추가로 해결.
- Commit: `29f702c`(퍼널-카드 연결 설명 추가).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일. ②③영역 주요 재구성 작업은 일단락, 추가
  피드백 대기.

### 2026-09-23(같은 세션 연속 9) / 진단·유지 요약카드 재배치
- 요구사항: "네"(④ 진단·유지도 같은 방식으로 이어서 진행). 어제 "레거시
  경로라 위험하다"며 보류했던 `awAuditLane`(6개월관리중 요약 4카드)이
  상세표 뒤에 있던 문제를, 오늘 확보한 재배치 패턴(단일 앵커 지점 변경)
  으로 재조사해 해결. 4개 서브탭 전체 DOM을 대조해 auditDraw/audit6m/
  auditClosedLoop/hd20AuditCloseEvaluation 카드가 전부 ".auditDraw
  기준" 체인으로 삽입되는 구조를 규명, auditDraw 자신의 앵커 한 곳만
  `.awHero`→`.awAuditLane`으로 바꿔 체인 전체를 자동으로 뒤로 이동.
  결과: "6개월 관리중"에서 요약 4카드가 상세표보다 먼저 표시, 다른 3개
  서브탭은 영향 없음, 콘텐츠 삭제 없이 순서만 조정(scrollHeight 불변).
- Commit: `a60d61d`(6개월 관리중 요약카드 재배치).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일 + ⑤ 개선실행 영역은 같은 방식 재검토 미착수.

### 2026-09-23(같은 세션 연속 10) / 개선실행 - 이전 수정의 부작용 교정
- 요구사항: "계속"(⑤ 개선실행도 같은 방식으로 이어서 진행). 4개 서브탭
  DOM 대조 중, 오늘 세션 초반 "검증현황 패널 미표시" 버그를 고치며 생긴
  부수효과(`.amHeroNote`가 `#hd20ActionVerifyStatus`에 밀려 순서가
  어긋남)를 발견. `ensureActionVerify`의 앵커 후보에 `.amHeroNote`를
  `.awHero`보다 우선 추가해 4개 서브탭 순서를 통일. ②③④⑤ 전 영역의
  "삭제 아닌 재구성" 재배치 작업이 이걸로 일단락됨.
- Commit: `19d2f5e`(효과재발관리 amHeroNote 위치 교정).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일. 추가 화면별 피드백 대기.

### 2026-09-23(같은 세션 연속 11) / 고도화 작업장 추이 전 영역 노출 버그
- 요구사항: "고도화작업장 추이란 제목의 차트가 중복처럼 보이는데?"
  조사 결과 index.html에 정적 중복은 없었으나, 이 카드가 `.awScreen`
  클래스가 없는 `.app` 직계 자식이라 영역 전환 로직(closeScreens/
  openScreen)의 관리 대상 밖에 있어 5개 영역 전부에서 항상 고정으로
  보이고 있었음을 확인(사용자가 "중복처럼 보인다"고 느낀 정확한 원인).
  `hd20-five-area-integration.js`에 기존 'hd20-nav-area-changed' 이벤트를
  재사용하는 작은 동기화 함수를 추가해, ③ 고도화·표준화 영역에서만
  보이도록 수정(콘텐츠 삭제 없음).
- Commit: `b4d45e1`(고도화 작업장 추이 영역 제한).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일. 추가 화면별 피드백 대기.

### 2026-09-23(같은 세션 연속 12) / 교차감사 + 조치목록 라벨 정리
- 요구사항: "계속"(트렌드차트 건 이후 유사 패턴 계속 점검). 5개 영역
  전체 교차감사로 추가 "새는" 요소가 없음을 확인. 이후 16개 탭 스크린샷
  재검토 중 ⑤ 조치 목록의 "진행/대기 241건"(상단)과 "진행중
  162건"(카드) 라벨 혼란을 발견 — 계산 검증 결과 데이터 오류가 아니라
  기한경과 포함 여부가 다른 두 지표였음. 삭제 대신 라벨을 "기한내
  진행중"으로 수정해 구분.
- Commit: `b0d8911`(조치목록 라벨 명확화).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일. 추가 화면별 피드백 대기.

### 2026-09-23(같은 세션 연속 13) / 효과재발관리 검증패널 중복 제거
- 요구사항: "계속"(스크린샷 재검토 지속). ⑤ 효과·재발관리에서 상단
  지표와 검증현황 패널의 5개 카드 중 3개(완료·효과검증대상/효과검증
  완료/재발확인)가 라벨까지 완전히 겹치는 것을 발견. hd20-ops-v2.js와
  renderActionVerify() 코드를 직접 대조해 같은 원본 데이터를 두 함수가
  각자 중복 계산하고 있었음을 확인. 중복 3개 카드를 제거하고 고유
  정보 2개(전체개선요청/조치진행대기)만 유지, 정보 손실 없이 정리.
- Commit: `f151a99`(검증패널 중복 카드 3개 제거).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일. 추가 화면별 피드백 대기.

### 2026-09-23(같은 세션 연속 14) / 전 16탭 자동 대조로 잔여 중복 소탕
- 요구사항: "계속". `find_remaining_dupes.py`로 16개 탭 전체 자동 대조
  결과, 조치목록 `.amSummary`가 지난 라벨 수정 이후에도 여전히 상단
  지표와 3/4 항목 값이 겹치는 것을 재확인. 4열 고정 그리드라 부분
  제거 시 레이아웃이 어색해지는 문제까지 고려해 카드 전체 삭제로
  정리(유일한 고유 정보는 "처리기간 분석" 탭이 이미 더 상세히 다룸).
- Commit: `d21fc0f`(조치목록 amSummary 완전 제거).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일. 자동 대조 스크립트로 16개 탭 전수 재검증
  완료, 남은 겹침 후보 없음. 추가 화면별 피드백 대기.

### 2026-09-23(같은 세션 연속 15) / 실적분석-활동관리 콘텐츠 미분화 발견
- 요구사항: "탭별 더 보완할 부분은 없는지 면밀히 검토해줘". 16개 탭을
  완성도(WORK PURPOSE 안내와 실제 콘텐츠 일치 여부) 관점에서 재검토,
  ② 실적분석이 활동관리와 완전히 동일한 화면을 보여주고 있음을 발견.
  원인은 유일한 분기 대상 요소(`.awRegisterFormCard`)가 항상 빈
  카드(모달로 실제 폼이 옮겨져 있음)라 토글이 무의미했던 것. 등록/
  가져오기 관련 4개 요소를 실적분석에서 숨기고 조회/분석 콘텐츠만
  남겨 두 서브탭을 실질적으로 구분.
- Commit: `2bbdcfa`(실적분석-활동관리 차별화).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일. 다른 탭들도 유사한 "안내와 콘텐츠 불일치"
  패턴이 있는지 계속 점검 필요.

### 2026-09-23(같은 세션 연속 16) / 대시보드 "성과·운영분석" 완성
- 요구사항: "탭별 더 보완할 부분은 없는지 면밀히 검토해줘" → CONTRACT
  전수 대조 중 "성과·운영분석"이 정의·계산로직은 있으나 진입 UI가
  전혀 없어 도달 불가능한 상태임을 발견 → 사용자 확인 후 "지금 준비된
  로직을 연결해서 실제로 만들어달라"는 요청으로 실제 구현 진행.
  1차로 `HD20_SUBNAV.select()`를 호출해 기존 `applyDashboard('analysis')`
  로직을 재사용하려 했으나 MutationObserver 연쇄로 브라우저가 멈추는
  문제를 발견해 되돌리고, `dashboard-section-tabs.js`의 안전한 targets
  방식만으로 6번째 섹션을 완성(다른 파일은 전혀 건드리지 않음). 대시보드
  영역에 이미 있던 "운영 상태" 블록을 재사용해 CONTRACT의 약속(Lead
  Time·유지율·재발률·조치완료율)을 충족.
- Commit: `2322352`(성과·운영분석 섹션 구현).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일. "성과·운영분석"이 오늘 세션 최초로 도달
  가능해짐 — 사용자 실사용 피드백 대기.

### 2026-09-23(같은 세션 연속 17) / "통합기준정보" 모달 전체 복원
- 요구사항: "계속" 검토 중 "통합기준정보" 버튼이 완전히 무반응임을 발견
  → 사용자 확인 후 "여러 파일을 역추적해서 모달 전체를 복원해달라"는
  요청으로 대형 복구 작업 진행. `#openMaster` 버튼·클릭핸들러·내용
  채우기 로직(4개 파일)은 전부 존재했으나 정작 `#masterModal` 컨테이너
  자체가 코드베이스 어디에도 없었음. 4개 파일의 셀렉터를 전수 대조해
  필요한 전체 구조(탭 2개+운영정책 자동추가, 순서편집 리스트, 분기별
  목표 입력 4개, 취소/저장)를 역산, 신규 파일 `master-info-modal.js`로
  뼈대를 작성해 app.js 로드 순서 앞에 배치. 3개 탭 전부 정상 작동,
  실제 저장까지 E2E로 검증.
- Commit: `4d7cfa0`(통합기준정보 모달 복원).
- 상세: 위 두 로그 파일 동일 문서에 이어서 기록.
- 잔여사항: 이전과 동일. "통합기준정보" 기능이 오늘 세션 최초로 정상
  작동하게 됨 — 사용자 실사용 피드백 대기.
